...
class WPCal_Availability_Date{
	...
	public function load(){
		...
		if( empty($result) ){
			throw new WPCal_Exception('availability_dates_id_not_exists');
		}
	...
	private function load_periods(){
		...
		$query = "SELECT `id` FROM `$table` WHERE `availability_date_id` = '". $this->get_id() . "' ORDER BY `from_time`";
	...
	public function set_from_date($value){
		$value = WPCal_DateTime_Helper::maybe_get_DateTime_obj($value);
	...
	private function calc_min_max_available_dates(){
		...
		elseif( $date_range_type === 'relative' ){
			$_min_date = new DateTime(date('Y-m-d'), wp_timezone());
			...
			$max_date->modify('+'.$relative_days.' days');
		}
		elseif( $date_range_type === 'infinite' ){
			...
			if($from_date instanceof DateTime){
				...
				$min_date = new DateTime(date('Y-m-d'), wp_timezone());
			}
