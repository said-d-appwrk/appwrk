...
class WPCal_Service_Availability_Slots{
	...
	private function _get_cached_data_details(DateTime $from_date, DateTime $to_date){
		...
		$query = "SELECT count(*) as num_of_days, min(`cache_created_ts`) as min_cache_created_ts FROM `$table_service_availability_slots_cache` WHERE `service_id` = '".$service_id."' AND `availability_date` >= '".$formated_from_date."' AND `availability_date` <= '".$formated_to_date."'";
	...
	private function remove_conflict_tp_calendar_events_collide_slots_for_a_day_by_admin(&$slots_for_a_day, DateTime $date_obj){
		...
		foreach($slots_for_a_day as $slots_for_a_day_key => $slot_obj){
			...
			foreach($events_for_a_day as $event){
				...
				$event_from_time = WPCal_DateTime_Helper::unix_to_DateTime_obj($event->from_time);
				$event_to_time = WPCal_DateTime_Helper::unix_to_DateTime_obj($event->to_time);
	...
	public function is_max_booking_per_day_reached(DateTime $date){
		...
		$query = "SELECT count(*) FROM `$table_bookings` WHERE `booking_from_time` >= '".$_from_time."' AND `booking_to_time` < '".$_to_time."' AND `status` = '1' AND `service_id` = '".$service_id."'";
	...
	private function is_slot_starts_within_min_schedule_notice(DateTime $slot_start_time){
		...
		if( $this->cache_absolute_min_schedule_notice === null ){
			...
			$absolute_min_schedule_notice = new DateTime('now', wp_timezone());
	...
	public function is_slot_still_available($booking_slot){
		...
		$booking_date = WPCal_DateTime_Helper::unix_to_DateTime_obj($booking_slot['from_time']);
	...
	private function get_cached_slots(DateTime $from_date, DateTime $to_date){
		...
		$query = "SELECT * FROM `$table_service_availability_slots_cache` WHERE service_id = '". $service_id . "' AND `availability_date` >= '". $from_date_formatted ."' AND `availability_date` <= '". $to_date_formatted ."' ORDER BY `availability_date`";
		...
		foreach($results as $result){
			...
			foreach ($result['slots'] as $current_slot) {
				...
				if( $this->is_slot_starts_within_min_schedule_notice( WPCal_DateTime_Helper::unix_to_DateTime_obj($current_slot['from_time'])) ){
					continue;
				}
	...
	function get_conflict_tp_calendar_events_of_admin_which_covers_this_date(DateTime $date_obj){
		...
		(
		(`from_time` >= '".$from_time_ts."' AND `from_time` < '".$to_time_ts."')
		OR
		('".$from_time_ts."' >= `from_time` AND '".$from_time_ts."' < `to_time`)
		)
