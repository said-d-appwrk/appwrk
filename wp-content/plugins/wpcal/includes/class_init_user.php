...
class WPCal_User_Init{
	...
	public static function enqueue_scripts() {
		...
		wp_enqueue_script( 'wpcal_user_chunk', WPCAL_PLUGIN_DIST_URL . 'js/chunk-vendors.js', [], WPCAL_VERSION, true );

		//if ( WPCal_Init::is_dev_env() ) {
			wp_enqueue_script( 'wpcal_user_chunk_common', WPCAL_PLUGIN_DIST_URL . 'js/chunk-common.js', [], WPCAL_VERSION, true );
		//}
		
		wp_enqueue_script( 'wpcal_user_app', WPCAL_PLUGIN_DIST_URL . 'js/user.js', [], WPCAL_VERSION, true );
		
		wp_localize_script('wpcal_user_app', 'wpcal_ajax', array('ajax_url' => admin_url('admin-ajax.php'), 'admin_url' => admin_url(), 'is_debug' => WPCAL_DEBUG));
	...
	private static function process_single_action($action, $request_data){
		...
		elseif($action === 'cancel_booking'){
			...
			$cancel_reason = sanitize_text_field($request_data['cancel_reason']);
